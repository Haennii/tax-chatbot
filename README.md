...
단순 LLM 호출에서 출발해 아래 문제들을 순서대로 발견하고 해결했습니다.

| 문제 | 해결 방법 |
|---|---|
| 세율표를 "가목/나목" 그대로 출력 | LLM 우회 → 박스문자 테이블 직접 파싱 |
| 조세특례제한법 조문 없음 | 법제처 API 대상 법령 확대 (시행령 3종 포함) |
| 항/호/목 내용 누락 | fetcher에 재귀 수집 로직 추가 |
| 키워드 불일치로 엉뚱한 조문 반환 | CONCEPT_MAP 도입 (개념 → 조문 직접 매핑) |
| 짧은 후속 질문이 엉뚱한 조문 검색 | 이전 질문과 합쳐 재검색하는 폴백 로직 추가 |
| 응답까지 40~60초 대기 | 스트리밍 응답(ask_stream) 적용 |
| 단발성 Q&A만 가능 | 대화 히스토리를 LLM에 전달해 멀티턴 구현 |
| 약어 검색 실패 (양도세, 부가세 등) | 동의어 확장 테이블 추가 |
| 조문이 길어 세부 조건 잘림 | 컨텍스트 전달 길이 1,000자 → 2,000자 확장 |
| 같은 조번호에 여러 조문 혼재 | CONCEPT_MAP에 article_title 필터 추가 |

---

## 한계점

로컬 소형 모델(7B)을 사용하기 때문에 다음과 같은 답변 오류가 발생할 수 있습니다.

**조문 내용 무시 문제**: 관련 조문을 정확히 찾아와도, 모델이 조문 내용 대신 자신의 학습 데이터를 기반으로 답변하는 경우가 있습니다. 예를 들어 성실신고확인대상사업자의 소득세 신고기한(6월 30일)을 일반 신고기한(5월 31일)으로 잘못 답변하는 식입니다.

**세부 조건 누락**: 긴 조문에서 모든 세부 조건을 빠짐없이 추출하지 못하는 경우가 있습니다.

> **해결 방향**: 검색 및 데이터 파이프라인은 정상 동작하며, 위 문제는 모델 교체로 개선 가능합니다. Claude API 등 대형 언어 모델을 연동할 경우 조문 기반 답변 정확도가 크게 향상될 것으로 예상됩니다.
